...
let dState: any; // deliverTx state
let cState: any; // commit state
 ...
 */
export async function startMain(deliverState: object, commitState: object, emitter: EventEmitter){
    try {
        dState = deliverState;
        cState = commitState;
    ...
    if(txObject.type === "OrderBroadcast"){
      ...
      try {      
        ...
        if (dState.mapping[recoveredAddr].orderBroadcastLimit > 0){
          ...
          Logger.consensus(`(Temporary log) Poster remaining quota:${dState.mapping[recoveredAddr].orderBroadcastLimit}`);
      ...
      if((dState.round.number === 0) && (txObject.data.round.number === 1)){
        ...
        if (txObject.data.round.number === (dState.round.number + 1)) {
          ...
          if (JSON.stringify(localMapping) === JSON.stringify(txObject.data.mapping)){
            ...
            Logger.consensus(`State proposal accepted for staking period #${dState.round.number}`);
            return Vote.valid("");
          } else {
    ...
    try {
      ...
      if (dState.round.number > cState.round.number) { //&& (rebalancer.getPeriodNumber() + 1 === state.round.number)) {
        ...
        if (dState.round.number > (1 + cState.round.number)) {
          Logger.consensusWarn("This round deliverTx state is more than 1 period ahead of commited state.");
        }
      ...
      stateHash = Hasher.hashState(cState); 

      Logger.consensus(`Commit and broadcast complete. Current state hash: ${stateHash}`);
    } catch (err) {
      console.log(err); // temporary
      Logger.consensusWarn("Error broadcasting orders (may require process termination).");
    }
